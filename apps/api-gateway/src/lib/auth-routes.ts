...
authRouter.get('/callback', async (req, res) => {
  ...
  if (error) {
    return res.redirect(`${APP_BASE_URL}/login?error=auth_error`);
  }
  if (!code || !state) {
    return res.redirect(`${APP_BASE_URL}/login?error=missing_params`);
  }

  const pkceKey = `jeffapp:pkce:${state}`;
  const storedRaw = await redisClient.get(pkceKey);
  if (!storedRaw) {
    return res.redirect(`${APP_BASE_URL}/login?error=invalid_state`);
  }
  ...
  if (!tokenResp.ok) {
    ...
    return res.redirect(`${APP_BASE_URL}/login?error=token_exchange_failed`);
  }
  ...
  try {
    ...
    return res.redirect(`${APP_BASE_URL}/login?error=invalid_id_token`);
  }
  ...
  if (!role) {
    return res.redirect(`${APP_BASE_URL}/login?error=access_denied`);
  }
  ...
  req.session.regenerate((err) => {
    if (err) return res.redirect(`${APP_BASE_URL}/login?error=session_error`);
    req.session.user = sessionUser;
    req.session.oidc = oidcData;
    if (mcpData) req.session.mcp = mcpData;
    req.session.save((saveErr) => {
      if (saveErr) return res.redirect(`${APP_BASE_URL}/login?error=session_error`);
